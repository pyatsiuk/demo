...
with sync_playwright() as p:
    ...
    page.pause()
    # Перший рядок — знаходимо рядок через Type і шукаємо Cost поруч
    row1 = page.get_by_role("textbox", name="Airport Taxes").locator("../../../..")
    row1.locator("awr-number input.awr-control-input").first.click()
    row1.locator("awr-number input.awr-control-input").first.fill("100")
    page.wait_for_timeout(300)

    # Другий рядок
    row2 = page.get_by_role("textbox", name="Catering").locator("../../../..")
    row2.locator("awr-number input.awr-control-input").first.click()
    row2.locator("awr-number input.awr-control-input").first.fill("200")
    page.wait_for_timeout(300)
